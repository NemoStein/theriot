package nemostein.games.geocontact.theriot.screens.gameplay.unitfactories
{
	...
	public class Unit extends AlphaEntity
	{
		...
		override protected function initialize():void
		{
			...
			health = stats.unitHealth.value * (Math.random() * 0.1 + 0.95);
			armor = stats.unitArmor.value * (Math.random() * 0.1 + 0.95);
			speed = stats.unitSpeed.value * (Math.random() * 0.1 + 0.95);
			power = stats.unitPower.value * (Math.random() * 0.1 + 0.95);
			range = stats.unitRange.value * (Math.random() * 0.1 + 0.95);
			rate = stats.unitRate.value * (Math.random() * 0.1 + 0.95);
			...
			addAnimation(FIRE, [3, 4, 5, 6, 0], 10, false, fireAnimationCallback);
		...
		override protected function update():void
		{
			if (!_dying)
			{
				...
				var willMove:Boolean = (!_target || _type == AIR) && !_reloading && _pathTracker;
				if (willMove)
				{
